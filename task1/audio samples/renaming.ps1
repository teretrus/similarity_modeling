...
[regex]$regex = "^(?<example>[0-9]+)[-](?<person>[0-9]+)[-](?<gender>[fm])[-](?<age>[0-9]+)[-][-](?<code>[0-9]+)(?<ext>[.]wav)$";
...
if ($file_count -eq 0) {
    ...
    $files | foreach-object {
        ...
        $new_name = [String]::Format("{0}-{1}-{2}-{3}-{4}-{5}{6}", "word", "E" + $match.Groups['code'], $match.Groups['example'], $match.Groups['person'], $match.Groups['gender'], $match.Groups['age'], $match.Groups['ext']);
